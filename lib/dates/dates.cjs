...
const calculateElapsedTime = (startTime, timeStamp) => {
  // if no timeStamp, startTime or if either of the values are not numbers (eg. Invalid), return empty string
  if (!(timeStamp && startTime && typeof timeStamp === 'number' && typeof startTime === 'number')) {
    return ''
  }

  const milliSeconds = new Date(timeStamp).getTime() - startTime
