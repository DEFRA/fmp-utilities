...
const formatReverseDate = (date) => { // eg 2025-04-17
  try {
    if (!date) {
      return ''
    }
    return new Date(offsetDateIfUTC(date, true)).toISOString().substring(0, 10)
  } catch {
    return ''
  }
}

const formatReverseUkDateTime = (date) => { // eg 2025-04-17
  try {
    if (!date) {
      return ''
    }
    return new Date(offsetDateIfUTC(date, true)).toISOString().substring(0, 19)
  } catch {
    return ''
  }
}
